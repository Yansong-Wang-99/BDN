...
class BDLIF(nn.Module):
    ...
    def forward(self, apical_input, basal_input, AP, h_leaf_apical, h_s4_apical,h_leaf_basal, h_s4_basal):
        ...
        return AP_burst, AP_raw, h_leaf_apical, h_s4_apical,h_leaf_basal, h_s4_basal, Ia, Ib
...
class BDN(nn.Module):
    def __init__(self, hidden_size=[64,32], in_dim=256, project_dim=256, out_dim=32):
        ...
        self.hidden_0 = BDLIF(hidden_size[0], project_dim=project_dim, feedback_dim=hidden_size[1],ratio=0.2)
        self.project_1 = nn.Linear(hidden_size[0], hidden_size[0])
        self.hidden_1 = BDLIF(hidden_size[1], project_dim=hidden_size[0], feedback_dim=out_dim,ratio=0.005)
    ...
    def forward(self, x: torch.Tensor, return_debug=False):  # [B, T, input_dim]
        B, T, _ = x.shape
        self.reset_net(B)
        y_0_list = []
        y_1_list = []
        ...
        for t in range(1,T+1):
            ...
            # apical_0_t = prev_y_1
            y_0_t, bAP_0_t, h0_leaf_apical, h0_s4_apical,h0_leaf_basal, h0_s4_basal, Ia0, Ib0 = self.hidden_0(apical_0_t, basal_0_t, prev_y_0, h0_leaf_apical, h0_s4_apical,h0_leaf_basal, h0_s4_basal)
            ...
            # apical_1_t = prev_y_out
            y_1_t, bAP_1_t, h1_leaf_apical, h1_s4_apical,h1_leaf_basal, h1_s4_basal, Ia1, Ib1 = self.hidden_1(apical_1_t, basal_1_t, prev_y_1, h1_leaf_apical, h1_s4_apical,h1_leaf_basal, h1_s4_basal)
            y_1_list.append(y_1_t)
            ia1_list.append(Ia1)
            prev_y_1 ,prev_y_out= y_1_t,bAP_1_t
            # prev_y_1 = y_1_t
            # output layer
        ...
        y_0_all   = torch.stack(y_0_list, dim=1)   # [B,T,64]
        y_1_all   = torch.stack(y_1_list, dim=1)   # [B,T,32]
        y_out_all = torch.stack(y_out_list, dim=1) # [B,T,32]
        ia0_all   = torch.stack(ia0_list, dim=1)   # [B,T,64]
        ia1_all   = torch.stack(ia1_list, dim=1)   # [B,T,32]
        if return_debug:
            # 只返回第一个样本的第一个t和最后一个t的各层输出
            debug = {
                'y_0_t0': y_0_all[0,0].detach().cpu().numpy(),
                'y_0_tN': y_0_all[0,-1].detach().cpu().numpy(),
                'y_1_t0': y_1_all[0,0].detach().cpu().numpy(),
                'y_1_tN': y_1_all[0,-1].detach().cpu().numpy(),
                'y_out_t0': y_out_all[0,0].detach().cpu().numpy(),
                'y_out_tN': y_out_all[0,-1].detach().cpu().numpy(),
                'ia0_t0': ia0_all[0,0].detach().cpu().numpy(),
                'ia0_tN': ia0_all[0,-1].detach().cpu().numpy(),
                'ia1_t0': ia1_all[0,0].detach().cpu().numpy(),
                'ia1_tN': ia1_all[0,-1].detach().cpu().numpy(),
            }
            return y_out_all, debug
        return y_out_all # [B,T,32]
