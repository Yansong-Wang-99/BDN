...
class SpikeBDN(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y):  # [B, T, input_dim]
        ...
        # y_out_all = F.sigmoid(y_out_all)

        if y is not None:
            # Hidden layers
            # neuromodulator_level = expectation(y).mm(self.NI.view(-1, prod(self.NI.shape[1:]))).view(y_0_t.shape)
            # mod_gradient = local_modulation(neuromodulator_level)
            ...
            # mod_gradient = torch.clamp(mod_gradient, min=-0.5, max=0.5)
            ...
            #         print(f"[Warning] 捕获到 Backward 错误 (已跳过): {error_msg}")
                    
            #         # 可选：如果发生错误，手动清空相关参数的梯度，防止脏数据污染
            #         if y_0_t.grad is not None:
            #             y_0_t.grad.zero_()
            #     else:
            #         # 如果不是 NaN 错误（比如显存不足等），还是抛出异常比较好
            #         raise e
