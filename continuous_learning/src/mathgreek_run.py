...
parser.add_argument('--experiment', default='mathgreek_classIL', type=str, required=False, choices=['mnist_classIL', 'cifar_classIL', 'gesture_classIL', 'alphabet_classIL', 'mathgreek_classIL'])
parser.add_argument('--approach', default='bdmlpnacasnn', type=str, required=False, choices=['sgd', 'ewc', 'naca', 'sgdsnn', 'ewcsnn', 'nacasnn', 'bdmlpnacasnn'])
...
parser.add_argument('--lr', default=1e-2, type=float, required=False) # 5e-4 is the best parameters for nacasnn in MNIST dataset
parser.add_argument('--lr_factor', default=0, type=float, required=False)
...
parser.add_argument('--sbatch', type=int, default=4)
...
parser.add_argument('--distribution', type=str, default='binary_tree', required=False, choices=['uniform', 'normal', 'beta', 'binary_tree'])
...
if args.approach == 'sgd':
    ...
    from approaches import ewc as approach
    from networks import mlp_snn as network
elif args.approach == 'bdmlpnacasnn':
    from approaches import naca as approach
    from networks import bdmlp_naca_snn as network
...
# Inits
def log_backward_nan(module, grad_input, grad_output):
    # grad_input: 这一层收到的梯度 (从后一层传回来的)
    # grad_output: 这一层计算完发出的梯度 (传给前一层的)
    
    # 检查输入的梯度是否有问题
    for i, g in enumerate(grad_input):
        if g is not None and (torch.isnan(g).any() or torch.isinf(g).any()):
            print(f"Layer {module} ({type(module).__name__}) received NaN/Inf in grad_input[{i}]")
            print(f"  - Shape: {g.shape}")
            # 你甚至可以在这里修改梯度，但通常只做打印

print('Inits...')
net = network.Net(args, inputsize, taskcla, labsize, nhid=args.nhid, nlayers=args.nlayers).cuda()
# === 关键修改：循环遍历模型中的所有层 ===
print("正在为所有层注册 NaN 监测钩子...")
for name, layer in net.named_modules():
    # 只给有参数的层或特定的层注册，避免报错
    # 也可以直接给所有层注册
    layer.register_full_backward_hook(log_backward_nan)
