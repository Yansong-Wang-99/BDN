...
import csv
from ncompt_bdn_vbilinear_mspacman_02_0005 import Agent, Args, make_env
...
# ------------------------
model_paths = [
    "./performance_ablation/MsPacmanNoFrameskip-v4__ncompt_bdn_vbilinear_mspacman_02_0005__2__1763436847_final.pt",
    "./performance_ablation/MsPacmanNoFrameskip-v4__ncompt_bdn_vbilinear_mspacman_02_0005__3__1763436856_final.pt",
    "./performance_ablation/MsPacmanNoFrameskip-v4__ncompt_bdn_vbilinear_mspacman_02_0005__26__1763435899_final.pt",
    "./performance_ablation/MsPacmanNoFrameskip-v4__ncompt_bdn_vbilinear_mspacman_02_0005__42__1764640584_final.pt",
    "./performance_ablation/MsPacmanNoFrameskip-v4__ncompt_bdn_vbilinear_mspacman_02_0005__101__1764247955_best.pt",
]
# noise_levels = [0.0, 0.01, 0.02, 0.03, 0.04]
noise_levels = [0.0,0.025,0.05,0.075, 0.1,0.125,0.15, 0.175,0.2]
...
def evaluate_with_noise(agent, args, device, noise_level, num_eval_episodes=10):
    ...
    env = gym.make(args.env_id, obs_type="grayscale")
...
if __name__ == "__main__":
    ...
    for noise_level in noise_levels:
        ...
        print(f"\n=== Evaluating noise_frac={noise_level:.2f} (sigma_pixel={sigma_pixel:.2f}) ===")
        ...
        for i, (agent, args) in enumerate(loaded_models):
            ...
            print(f" Model {i+1}: mean_return={mean_return:.2f}, std={std_return:.2f}")
            model_returns.append(mean_return)

        mean_across = float(np.mean(model_returns))
        std_across = float(np.std(model_returns))
        print(f" -> Average across {len(model_returns)} models: {mean_across:.2f} (std {std_across:.2f})")
