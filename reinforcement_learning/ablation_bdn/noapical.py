...
class DendriteNode(nn.Module):
    ...
    def forward(self, leaf_in, bAP, prev_h_leaf, prev_h4):
        ...
        alpha_leaf = torch.exp(-1.0 / tau_leaf)  # [1,64,3,1] # TODO decay ablation
        # tau_s4 = F.softplus(self.log_tau_s4)  # [1,64,1]
        tau_s4 = self.max_tau * torch.sigmoid(self.tau_s4) + 1
        alpha_s4 = torch.exp(-1.0 / tau_s4)  # TODO decay ablation
        # 1) bAP
        ...
        # 2) 叶子输入
        h_leaf_new = alpha_leaf * h_leaf_bAP + (1 - alpha_leaf) * leaf_in  # [B,64,3,4]
        # 3) 叶子非线性
        ...
        h4_new = alpha_s4 * h4_bAP + (1 - alpha_s4) * u4
        # 5) dendrite输出
...
class BDLIF(nn.Module):
    ...
    def forward(self, basal_input, AP,  h_leaf_basal, h_s4_basal):
        ...
        AP_raw = self.soma(Is)
        AP_burst = AP_raw
        return AP_burst, AP_raw,  h_leaf_basal, h_s4_basal
...
class BDN(nn.Module):
    ...
    def forward(self, x: torch.Tensor):  # [B, T, input_dim]
        ...
        h0_leaf_basal, h0_s4_basal = None, None
        h1_leaf_basal, h1_s4_basal = None, None,
        for t in range(1, T + 1):

            x_t = x[:, t - 1, :]
            basal_0_t = self.project_0(x_t)

            y_0_t, bAP_0_t, h0_leaf_basal, h0_s4_basal = self.hidden_0(basal_0_t,prev_y_0,h0_leaf_basal,h0_s4_basal)
            prev_y_0 = bAP_0_t
            basal_1_t = self.project_1(y_0_t)
            y_1_t, bAP_1_t,h1_leaf_basal, h1_s4_basal = self.hidden_1(basal_1_t,prev_y_1,h1_leaf_basal,h1_s4_basal)
